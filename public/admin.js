...
function showSvcForm(){document.getElementById('svc-form').style.display='block';['sv-name','sv-price','sv-dur','sv-desc'].forEach(id=>document.getElementById(id).value='');delete document.getElementById('svc-form').dataset.editId}
async function saveService(){
  try{
    ...
    const body={name:n,price:p,duration:d,description:document.getElementById('sv-desc').value.trim()};
...
async function editService(id){try{const r=await fetch(A+'/api/admin/services',{headers:H});const d=await r.json();const s=d.find(x=>x.id===id);if(!s)return;document.getElementById('sv-name').value=s.name;document.getElementById('sv-price').value=s.price;document.getElementById('sv-dur').value=s.duration;document.getElementById('sv-desc').value=s.description||'';document.getElementById('svc-form').dataset.editId=id;document.getElementById('svc-form').style.display='block'}catch(e){}}
